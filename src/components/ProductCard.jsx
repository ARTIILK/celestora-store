...
import { ExternalLink, Sparkles, Package, ChevronRight } from "lucide-react";
  ...
  return (
    <>
      <motion.div
        ...
        className="group relative bg-card/90 backdrop-blur-md rounded-lg border border-white/10 overflow-hidden hover-card-fx glow-border cursor-pointer flex flex-col h-full"
      >
        {/* Featured Badge */}
        <div className="absolute top-6 left-6 z-20 flex flex-col gap-2">
          {product.featured && (
            <div className="bg-primary/20 backdrop-blur-md text-primary text-[10px] tracking-[0.2em] font-black px-4 py-2 rounded-lg flex items-center border border-primary/30 uppercase">
              <Sparkles className="w-3 h-3 mr-2" />
              Featured
            </div>
          )}
          {product.stock && (
            <div className={`text-[10px] tracking-[0.2em] font-black px-4 py-2 rounded-lg border uppercase backdrop-blur-md ${
        ...
        <div className="relative aspect-square w-full overflow-hidden flex items-center justify-center p-10">
          ...
          {hasImage ? (
            <img
              ...
              onError={(e) => {
                e.target.src = 'https://ui-avatars.com/api/?name=' + product.name + '&background=random&size=256';
              }}
            />
          ) : (
            ...
            <div className="col-span-2 bg-black/40 relative flex items-center justify-center p-8 min-h-[250px] sm:min-h-full border-b sm:border-b-0 sm:border-r border-white/5">
              {hasImage ? (
                <img
                  src={product.image}
                  alt={product.name}
                  className="w-full object-contain drop-shadow-[0_0_30px_hsl(var(--primary)/0.3)]"
                  onError={(e) => {
                    e.target.src = 'https://ui-avatars.com/api/?name=' + product.name + '&background=random&size=256';
                  }}
                />
